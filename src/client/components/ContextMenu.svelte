<script>
    ...
    $: x =
        $contextMenu.pos.x + 200 > window.innerWidth
            ? $contextMenu.pos.x - 150
            : $contextMenu.pos.x;
    $: y =
        $contextMenu.pos.y + 200 > window.innerHeight
            ? $contextMenu.pos.y - 150
            : $contextMenu.pos.y;
...
<ul
    ...
    bind:this={cmenu}
>
    <li class="divider" data-content={$contextMenu.item.name} />
    {#each menu as link}
        <li class="menu-item">
            <a
                href="#_"
                on:click|preventDefault|stopPropagation={(link.action(index),
                close)}
            >
                <i class="menu-icon icon icon-{link.icon} {link.class}" />
    ...
    <li class="menu-item">
        <a
            href="#menus"
            on:click|preventDefault={() => ($contextMenu.isOpen = false)}
        >
            <i class="menu-icon icon icon-cross text-gray" />
